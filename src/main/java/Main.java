...
public final class Main {
  ...
  public static class CubePipeline implements VisionPipeline {
    ...
    public void process(Mat source0) {
      ...
      cvDilate(cvDilateSrc, cvDilateKernel, cvDilateAnchor, cvDilateIterations, cvDilateBordertype, cvDilateBordervalue,
          cvDilateOutput);
    ...
    private void filterContours(List<MatOfPoint> inputContours, double minArea,
        double minPerimeter, double minWidth, double maxWidth, double minHeight, double maxHeight, double[] solidity,
        double maxVertexCount, double minVertexCount, double minRatio, double maxRatio, List<MatOfPoint> output) {
    ...
    if (cameras.size() >= 1) {
      VisionThread visionThread = new VisionThread(cameras.get(0), new ConePipeline(), pipeline -> {
        if (!(pipeline.filterContoursOutput().size() == 0)) {
          ...
          Point[] bBoxPts = { new Point(rectB.x, rectB.y), new Point(rectB.x + rectB.width, rectB.y),
              new Point(rectB.x + rectB.width, rectB.y + rectB.height), new Point(rectB.x, rectB.y + rectB.height) };
        ...
        if (!(Main.cubePipeline.filterContoursOutput().size() == 0)) {
          List<MatOfPoint> squareShapedContourList = new ArrayList<MatOfPoint>();
          for (MatOfPoint contour : Main.cubePipeline.filterContoursOutput()) {
            RotatedRect rotatedRectTemp = Imgproc.minAreaRect(new MatOfPoint2f(contour.toArray()));
            if (!(rotatedRectTemp.size.height / rotatedRectTemp.size.width > 0.985
                && rotatedRectTemp.size.height / rotatedRectTemp.size.width < 1.015)
                || !(rotatedRectTemp.size.width / rotatedRectTemp.size.height > 0.985
                    && rotatedRectTemp.size.width / rotatedRectTemp.size.height < 1.015))
              continue;
            squareShapedContourList.add(contour);
          }

          MatOfPoint largestMatrixCube;
          if (squareShapedContourList.size() != 0) {
            largestMatrixCube = squareShapedContourList.get(0);
            for (MatOfPoint contour : squareShapedContourList) {
            ...
            Point[] bBoxPtsCube = { new Point(rectBCube.x, rectBCube.y),
                new Point(rectBCube.x + rectBCube.width, rectBCube.y),
                new Point(rectBCube.x + rectBCube.width, rectBCube.y + rectBCube.height),
                new Point(rectBCube.x, rectBCube.y + rectBCube.height) };
            ...
            Imgproc.polylines(Main.original /* the original image */,
                ...
                true /* Is a Closed Polygon? */,
                new Scalar(124, 252, 0), /* For RGB Values of Box */
                1,
                Imgproc.LINE_4 /* Line type */);
            Imgproc.polylines(Main.original,
                bListMidContourCube,
                true,
                new Scalar(255, 95, 31),
                1,
                Imgproc.LINE_4);
            ...
            SmartDashboard.putBoolean("Cube Pipeline Seeing Object", true);
          ...
          SmartDashboard.putNumber("Cube X", 0.0d);
          SmartDashboard.putNumber("Cube Y", 0.0d);
          SmartDashboard.putBoolean("Cube Pipeline Seeing Object", false);
          SmartDashboard.putBoolean("Cube in Vision", false);
          Main.outputStreamCube.putFrame(Main.original);
        }
      });
